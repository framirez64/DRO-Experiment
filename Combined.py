...
def EachFrameChecker(Stage):
    ...
    #COULD I CLICK THE BUTTON WHILE IT"S NOT SEEN?
    if Phase == "Break":
        CircleOn = 0
        RedCircleOn = 0
        ...
        CircleOn = 0
        RedCircleOn = 0
    #This piece of code goes with Interval Data 
    #if DataTimer.getTime() > 60:

    #take data every 60 seconds
    #TakeIntervalData(PTime,RTime)
        ...
        if Phase == "BL" or Phase == "Assessment":
            if Reinforcement_Schedule == "FI":
                if FITimer.getTime() > Reinforcement_Variable:
                    ...
                    FITimer.reset()
            elif Reinforcement_Schedule == "FR":
...
text = visual.TextStim(win=win, name='text',
                       text='Your goal is to earn as many points as possible. How you earn points may change '
                            'throughout the experiment.'
                            'All your points will be visible throughout the experiment at the top of the screen. You '
                            'will be given'
                            'optional 1-minute breaks every 5 minutes and a 10-minute break halfway through the '
                            'experiment (after'
                            'approximately 30 minutes).You may skip past the breaks by clicking the button that '
                            'appears on the screen.'
                            'You are competing with other people for the most points; the top three point-earners in '
                            'the study will win'
                            'an additional gift card. Do your best to earn the most points!  Click anywhere when you '
                            'are ready and good'
                            'luck',
                       font='Open Sans',
...
# --- Potentially replaced with a class Object
...
BreakTxt = visual.TextStim(win=win, name='BreakTxt',
                           text='Break \n' + str(
                               Duration - PhaseTimer.getTime()) + 'If you wish to skip the break, click the orange '
                                                                  'square below',
                           font='Open Sans',
                           pos=(0, 0), height=0.05, wrapWidth=None, ori=0.0,
                           color='white', colorSpace='rgb', opacity=None,
...
InitializeComponents = [text, mouse]
...
if thisPhaseSelector is not None:
    ...
    while continueRoutine:
        ...
        if DROMouse.status == STARTED:  # only update if started and not finished!
            ...
            if buttons != prevButtonState:  # button state changed?
                ...
                if sum(buttons) > 0:  # state changed to a new click
                    ...
                    clickableList = environmenttools.getFromNames([DROCircle, RedCircle, BreakButton],
                                                                  namespace=locals())
                    for obj in clickableList:
                        # is this object clicked on?
        ...
        if BreakTxt.status == STARTED:
            ...
            if Duration > PhaseTimer.getTime():
                BreakTxt.text = 'hey you can take a break now! You have ' + str(round(
                    Duration - PhaseTimer.getTime())) + ' seconds left in your break, \n however you can click the button to end it early \n'
